...
def get_validation_data(ft, lb, val_pct):
    ...
    :param ft: input features
    :param lb: labels
    :param val_pct: percentage of traning set used for validation"""
    # Shuffle
    nb_examples = len(ft)
...
class generator(object):
    """Data generator sorting {features,labels} sets and batching"""
    ...
    def change_contrast(self, arr):
        ...
        # convert initially normalized array
        arr = (arr * 255).astype(np.uint8)
        # clip pixel values within [0,255] range
    ...
    def augment_magnitude(self, arr):
        """Color augmentation"""
        # select mode
        functions = [self.change_brightness,
    ...
    def next_batch(self, batch_size, data_aug=True):
        ...
        for i in range(start, end):
            ...
            if data_aug is True:
                ...
                # /!\ clipped_zoom() not stable given the scale factor used
                # you MUST check results visually after changing the coefficients
            ...
            for img_ in img_name:
                ...
                # normalize RGB by 255, otherwise normalize data by min-max scaling
                # given that no-data values are 0s while the true minimum is beyond 0
                img = np.multiply(img, 1.0 / 255.0) if len(img.shape) == 3 \
                    else ((img - np.min(img[img != 0])) / (np.max(img) - np.min(img[img != 0]))).clip(min=0)
            ...
            for nm in anno_names_:
                ...
                anno = self.augment_geometry(np.expand_dims(anno, axis=-1)) \
                    if data_aug is True else np.expand_dims(anno, axis=-1)
...
def read_data_sets(img_path, anno_path, img_size, val_pct):
    """Load and split (randomly) input data into validation and training sets
    :param img_path: path to input images
    :param anno_path: path to annotations
    :param img_size: image size
    :param val_pct: percentage of data used for validation"""
    # get input names
    ...
    try:
        img_train, anno_train, img_valid, anno_valid = get_validation_data(ft=img_names,
                                                                           lb=anno_names,
                                                                           val_pct=val_pct)
    except IndexError:
        print('The size of inputs/annotation sets must be equal.')

    # shuffle loaded imgages and annotations
    ...
    # create generators with batching system
